...
def laplacian_periodic(Z: np.ndarray, inv_dx2: float) -> np.ndarray:
    ...
    return (np.roll(Z, 1, 0) + np.roll(Z, -1, 0) +
            np.roll(Z, 1, 1) + np.roll(Z, -1, 1) - 4.0 * Z) * inv_dx2
...
def laplacian_neumann(Z: np.ndarray, inv_dx2: float) -> np.ndarray:
    ...
    return (Zp[:-2, 1:-1] + Zp[2:, 1:-1] +
            Zp[1:-1, :-2] + Zp[1:-1, 2:] - 4.0 * Z) * inv_dx2

def compute_laplacian(Z: np.ndarray, dx: float, bc: Literal["periodic", "neumann"]) -> np.ndarray:
