...
def save_animation(
    ...
    im_u = axes[0].imshow(
        U0.T, origin='lower', cmap=CMAP_U, 
        vmin=0, vmax=1.0, animated=True
    )
    axes[0].set_title("Substrate U")
    
    im_v = axes[1].imshow(
        V0.T, origin='lower', cmap=CMAP_V, 
        vmin=0, vmax=1.0, animated=True
    )
    ...
    try:
        ...
        logger.error(f"Failed to save animation. Ensure ffmpeg is installed if using mp4.\nError: {e}")
    finally:
        plt.close(fig)